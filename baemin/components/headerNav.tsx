...
import { CartContext, CartProvider } from "../contexts/cart/cartContext";
...
export default function HeaderNav() {
    ...
    const onSearch: SearchProps['onSearch'] = (value, _e, info) => {
        router.push(`/sreach?keyword=${value}`);
    };
    const navigation = () => {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                const response = await fetch('http://localhost:8080/foods/categories');
        ...
        <CartProvider>
            <div className="w-full h-fix bg-white flex flex-row fixed  py-3 gap-4 justify-items-center	justify-center z-50	">
                ...
                <div className="grow  flex flex-row items-center gap-9	 ">
                    <Select className="ml-10 w-28" defaultValue="Tất cả" options={options.map((option) => ({ label: option, value: option }))}></Select>
                    <Search
                        className="w-1/3"
                ...
                <div className="flex-none w-1/4  flex flex-row items-center  py-2" >
                    ...
                    <span className="text-xs bg-red-600 relative rounded w-full text-white  bottom-3 right-4 text-center" style={{ width: '15px', borderRadius: '50px' }}  >{cart && cart.length >= 1 && cart.length}</span>
                </div>
