...
const Page: React.FC = () => {
    ...
    const status: any = cart.map((item: any) => {
        return {
            id: item.id,
            number: item.number,
            name: item.status === 'paid' ? 'Nhà hàng đã nhận đơn' : 'Shipper đang giao hàng',
            st: false
        }
    })
                        ...
                        <div className='w-full flex flex-row'>
                            ...
                            <div className='w-1/3 flex flex-col  gap-2 pl-5'>
                                <div className='font-medium flex flex-row justify-between '>
                                    ...
                                    <span className='text-beamin'>{cartTotal}</span>
                                </div>
                                ...
                                <div className='text-beamin w-full flex flex-row items-end justify-end text-xl font-medium pr-3 pt-3'>
                                    <span>{
                                        cartTotal + 38.000 + 16.000
                                        }d</span>
                                </div>
                            </div>
                        </div>
                        <div className='w-full mt-2 border-t'>
                            <DetailsCheckout items={
