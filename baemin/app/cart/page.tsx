...
export default function Home() {
    const { cart, clearCart, cartTotal, onCheckout } = useContext(CartContext);
        ...
        <div className="mt-4 px-16 flex flex-col gap-4  pb-16 rounded-md">
            ...
            <div className=" flex flex-row fixed bottom-0  w-[90.6%]  mr-16  h-16 bg-white items-center  " >
                <div className="flex flex-row gap-2 w-1/2 h-full items-center ml-10">
                    <div className="cursor-pointer hover:text-red-600 " onClick={clearCart}>Hủy</div>
                    <div> Quán Đã chọn: </div>
                    <div> The Chicken Gang</div>
                </div>
                <div className="flex flex-row gap-2 w-1/2 h-full items-center justify-end pr-2">
                    <div className=""> {`Tổng thanh toán (${cart.length} Sản phẩm):`}
                    </div>
                    <div className="text-red-600" >₫{cartTotal} </div>
                    <div>
                        <Button style={{ 'background': '#3AC5C9', color: 'white' }} className="bg-beamin text-white w-40 h-10 rounded-md hover:brightness-105" onClick={() => onCheckout()} >Thanh toán</Button>
                    </div>
                </div>
            </div>
        </div>
